...
void Utility::deleteSCCs(SCC_Set* SCCs){
    ...
    for(std::unordered_set<Vid>* set: *SCCs)
        delete set;
    delete SCCs;
}
    ...
    for(int cluster = 0; cluster < numClusters; cluster++){
        ...
        for(int v = 0; v < clusterSize; ++v){
            ...
            while(edge++ < numNeighbors){
                ...
                graph->insertEdge(cluster + v*numClusters, x*numClusters + cluster);
            }
        ...
        for(int j = 0; j < interClusterConnections; ++j){
            ...
            w = Utility::randomInt(0, clusterSize)*numClusters +clust;
            graph->insertEdge(j*numClusters + cluster, w);
